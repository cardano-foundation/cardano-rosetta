...
import {
  ...
  CONSTRUCTION_UNSIGNED_TRANSACTION_WITH_EXTRADATA
} from '../fixture-data';
...
describe(CONSTRUCTION_PARSE_ENDPOINT, () => {
  ...
  test('Should return 1 input, 2 outputs and signers if a valid signed transaction is set', async () => {
    const response = await server.inject({
      ...
      payload: generateParsePayload('cardano', 'mainnet', true, CONSTRUCTION_SIGNED_TRANSACTION_WITH_EXTRADATA)
    });
  ...
  test('Should return 1 input, 2 outputs and empty signers if a valid unsigned transaction is set', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('cardano', 'mainnet', false, CONSTRUCTION_UNSIGNED_TRANSACTION_WITH_EXTRADATA)
    });
  ...
  test('Should throw an error when valid unsigned transaction bytes but signed flag is true are provided', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('cardano', 'mainnet', true, CONSTRUCTION_UNSIGNED_TRANSACTION_WITH_EXTRADATA)
    });
    expect(response.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
    expect(response.json()).toEqual({
      code: 4011,
  ...
  test('Should throw an error when valid signed transaction bytes but signed flag is false are provided', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('cardano', 'mainnet', false, CONSTRUCTION_SIGNED_TRANSACTION_WITH_EXTRADATA)
    });
    expect(response.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
    expect(response.json()).toEqual({
      code: 4012,
